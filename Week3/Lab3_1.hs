...
contradiction form = not $ any (\x -> evl x form) $ allVals form
...
{- a statement or formula is a tautology if evaluating it with any x returns true,
 therefore, we're checking if all evaluations result in formula being true -}
tautology :: Form -> Bool
...
{- form1 entails form2, if for every (all) x, evaluating form1 with x is true, then evaluating form2 with x is also true.
   Simply checking if for every evaluation, f --> g holds -}
entails :: Form -> Form -> Bool
entails form1 form2 = and $ zipWith (-->) (map (\x -> evl x form1) $ allVals form1) (map (\x -> evl x form2) $ allVals form2)
