...
{-
testR :: Int -> Int -> ([a] -> [a] -> Bool) -> IO()
testR k n p = if k == n then print (show n ++ " tests passed")
                else do
				  xs <- list1
				  ys <- list2
                  if p xs xs then
                    do print ("pass one test on iteration number:" ++ show k)
                       testR (k+1) n p
                  else error ("failed test on iteration number:" ++ show k)

testProp :: ([a] -> [a] -> Bool) -> IO()
testProp p = testR 1 100 p -- test property 'p'. Start with counter = 1 and test up to n = 100 times 
-}
